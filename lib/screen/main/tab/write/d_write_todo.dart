...
import 'package:fast_app_base/screen/main/tab/write/vo_write_to_result.dart';
...
class _WriteTodoDialogState extends DialogState<WriteTodoDialog>
  ...
  DateTime _selectedDate = DateTime.now(); //값이 변하니 final은 붙이지 않음
  ...
  void initState() {
    if (widget.todoForEdit != null) {
      _selectedDate = widget.todoForEdit!.dueDate; //not null 의미의 느낌표
              ...
              RoundButton(
                  ...
                  onTap: () {
                    widget.hide(WriteTodoResult(
                        _selectedDate, textController.text)); //결과 반환
                  })
            ],
          )
        ],
